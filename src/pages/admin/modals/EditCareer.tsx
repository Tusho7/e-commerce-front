...
interface EditCareerProps {
  career: AboutUsProps;
  onClose: () => void;
  onSave: (id: number, updatedData: CareerProps) => void;
}

const EditCareer: React.FC<EditCareerProps> = ({ career, onClose, onSave }) => {
  const [title, setTitle] = useState(career.title);
  const [content, setContent] = useState(career.content);

  const handleSave = () => {
    onSave(career.id, { id: career.id, title, content });
  };
