...
import { createTerms } from "../../../services/terms";

interface AddTermsProps {
  onClose: () => void;
  onAdd: (newData: TermsProps) => void;
}

const AddTerms: React.FC<AddTermsProps> = ({ onClose, onAdd }) => {
  const [title, setTitle] = useState<string>("");
  const [content, setContent] = useState<string>("");

  const handleAdd = async () => {
    try {
      const newTerms: TermsProps = { id: 0, title, content };
      await createTerms(newTerms);
      onAdd(newTerms);
      Swal.fire({
        title: "Success!",
        text: "Terms section added successfully",
        icon: "success",
        timer: 1500,
      ...
      console.error("Failed to add terms data:", error);
      Swal.fire({
        title: "Error!",
        text: "Failed to add Terms section",
        icon: "error",
        timer: 1500,
        showConfirmButton: false,
      });
